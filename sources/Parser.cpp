...
{
    auto types = vchips::ComponentFactory::getFactory()->getNames();
...
void Parser::isValidLine(const std::string &line)
{
    ...
    if (!line.compare(".chipsets:")) {
        if (_isChips)
            throw vchips::SyntaxError("multiple \'.chipsets:\' definition.");
        _isChips ? (_isChips = false) : (_isChips = true);
    }
    if (!line.compare(".links:")) {
        if (_isLinks)
            throw vchips::SyntaxError("multiple \'.links:\' definition.");
    ...
    if ((std::regex_match(line, match, chipReg)) || (std::regex_search(line, match, linkReg))) {
        if (!_isLinks && _isChips) {
            if (!isValidType(match[1]))
                throw vchips::SyntaxError("\'" + std::string(match[1]) + "\' component type is unknown.");
            else if (isAlreadyAdded(match[2]))
                throw vchips::SyntaxError("multiple \'" + std::string(match[2]) + "\' definition.");
            else
...
void Parser::isValidFile()
{
    ...
    if (!_isChips || !_isLinks) {
        if (!_isChips)
            throw vchips::SyntaxError("no \'.chipsets:\' section found.");
        if (!_isLinks)
            throw vchips::SyntaxError("no \'.links:\' section found.");
    }
    for (auto [type, name, param] : _chipsets)
        if (!type.compare("output") && !isLinked(name))
            throw vchips::SyntaxError("Output \'" + name + "\' is not linked to anything.");
}

void Parser::isValidFilePath(const std::string &filePath)
{
    if (!std::filesystem::exists(filePath))
        throw vchips::FileError("\'" + filePath + "\' doesn't exist.");
    if (std::filesystem::is_directory(filePath))
        throw vchips::FileError("\'" + filePath + "\' is a directory.");
}
...
void Parser::parseFile(const std::string &filePath)
{
    ...
    } catch (vchips::FileError const &error) {
        std::cerr << program_invocation_short_name << ": " << error.what() << std::endl;
        exit(84);
    }
